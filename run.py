...
def batch_fit(batch, model, loss_func, mode='train'):
    ...
    if mode == 'train':
        ...
        return loss, [regre_loss, cl_loss, sup_cl_loss, sup_cl_loss_m, sup_cl_loss_t, sup_cl_loss_v, sup_cl_loss_a,
                      self_cl_loss, self_cl_loss_m, self_cl_loss_t, self_cl_loss_v, self_cl_loss_a, intra_loss,
                      inter_loss], \
...
def train(train_loader, valid_loader, test_loader, model, regre_loss_func, scl_loss_func, optimizer, scheduler):
    ...
    for epoch in tqdm(range(config["epoch"]), desc="回归模型训练中..."):
        ...
        train_sup_cl_loss, train_sup_cl_loss_m, train_sup_cl_loss_t, train_sup_cl_loss_v, train_sup_cl_loss_a = [
                                                                                                                    0.0] * 5
        train_self_cl_loss, train_self_cl_loss_m, train_self_cl_loss_t, train_self_cl_loss_v, train_self_cl_loss_a = [
                                                                                                                         0.0] * 5
...
def run_msa(seed):
    ...
    if config["do_train"]:
        ...
        # if not device == "cpu":
        #    regression_model = nn.DataParallel(regression_model, device_ids=device)
        regression_model = regression_model.to(device)

        bert_no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.weight']
        bert_params = list(regression_model.encoder_t.named_parameters())

        bert_params_decay = [p for n, p in bert_params if not any(nd in n for nd in bert_no_decay)]
        bert_params_no_decay = [p for n, p in bert_params if any(nd in n for nd in bert_no_decay)]
        other_params = [p for n, p in list(regression_model.named_parameters()) if 'encoder_t' not in n]
        ...
        scheduler_except_bert = ReduceLROnPlateau(optimizer_except_bert, mode='min', patience=5, factor=0.1,
                                                  verbose=True)
